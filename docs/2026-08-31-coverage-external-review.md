...
> **校正區塊（2026-09-01）：本檔有三處已被後續實作推翻，原文保留不改,以免這份紀錄
> 自己變成「事後看起來一直都對」的版本。**
>
> 1. 下表 C4 標「產品 bug，尚未修」——**已修**。`3fd9d9bd` 讓 `crate::`／`self::`／`super::`／`::`
>    限定呼叫退回最後一段，`209fa06f`＋`8d43628c` 再加上 module-path 後綴解析。
>    測試在 `rust/tests/impact.rs:187,240` 與 `rust/tests/graph.rs:797,821,843`。
> 2. 「由這裡定的下一步（旗標重新設計，尚未實作）」那一節——**四項全部已實作**：
>    `scan_skipped` 翻旗標（`coverage.rs:494-535`）、`unparsed` 降為 advisory（`:536,668-672`）、
>    「讀列不要讀布林值」措辭（`:699`／README #11／skill）、宣告行不再標 `call`（`:288-294`）。
> 3. 「**建議：不做 B**」——**已翻案**。`209fa06f` 為 Rust 加了 `edge:imports`。當初判定「不做」時
>    想的機制是「退回末段再全域比名字」，實作的版本比的是**模組路徑**，兩者不是同一件事。
>    教訓已記在 `docs/2026-08-31-recall-wip.md:172-176`：判定不做時要寫清楚是**哪種機制**不做。
>
> 另補一項本檔尚未記、但屬於同一條線的已知缺口:`edge:imports` 連出來的邊**沒有任何查證面**——
> `verify.rs:188-190` 說 import 邊沒有呼叫點、不進 `line_precision`,而 `coverage.rs:317,368` 的
> L2 只查 `rel_type = 'calls'`。也就是說那批邊兩個螢幕都看不到。
> **後續(2026-09-01 同日)**:先實測根因,發現這條比寫下來的更根本——import 邊**從不成
> relationship**(頂層 `use` 無源 chunk;本倉庫 336 條 raw、0 條 relationship),所以「連錯」不可能,
> 真洞是「丟掉不可見」。L2 已納入 imports(檔案層級抑制、braces 開展),丟掉的 import 現在以
> `drop` 列帶 use 行現形;`verify.rs` 的排除改成註解講明現狀,並寫死「若 import 日後成邊,其 use 行
> 可逐行查證,不得延用此排除」。詳見 `docs/2026-08-31-recall-wip.md` §1 對應列。
