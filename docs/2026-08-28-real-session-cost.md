...
> ### 2026-08-31 校正（必读）
>
> 本文的**方向**在獨立樣本上重複成立了,但它的**證據基礎已不存在**:§1.1 與 §2 引用的
> finance-engineering(245 MB/764 檔)與 claw-skills(47 MB/57 檔)transcript,已被 Claude Code 的
> 30 天留存策略刪除(`~/.claude/.last-cleanup` = 2026-08-31T01:52Z),目前 `~/.claude/projects`
> 總量 112 MB 且無這兩個目錄。文中 1,565 筆輸入、27 筆命中、14,731 次工具呼叫**都無法重算**。
>
> 重新量過、且可重算的版本在 `docs/2026-08-31-demand-recheck.md`(工具 `cort-evals demand`,
> 產物 `evals/runs/2026-08-31-demand/`)。它的結論比本文多了一件重要的事:**用戶幾乎不問關係
> (1,214 筆裡 1 筆問句),但 agent 自己每天都在做呼叫點枚舉,而且常做錯、常被要求查證**
> (42% 的往來是把 agent 報告貼回來查)。所以 §1.1 的「沒有需求」請讀成「沒有*問句*需求」;
> 需求存在,只是它長在刪除/重構/審閱的協作路上,而 §1.4 那個「cort 讀不了 Rust」的限制已被
> 後來的 Rust pack 處理。
