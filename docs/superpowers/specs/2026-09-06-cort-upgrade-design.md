...
## 9. 首輪審查的五條:已於 2026-09-06 逐條核實,全部成立

計畫三開工前的功課,因為其中四條正好落在計畫三要動的表面上。原本這一節寫的是「未核實,故不納入」——
那等於把五個可能的缺陷放在一個沒有人負責的位置。現在每一條都附驗證方式與後果。

| # | 事實 | 核實 | 對計畫三的意義 |
|---|---|---|---|
| 1 | **manifest 在「宣稱先於任何變動」的 preflight 之前就被改了。** `migrate_manifest_v2` 在 `install.sh:1083`,而那句 `Preflight every destination ... before any mutation` 的註解在 `:1087`。 | 讀行號即證 | preflight 若中止(來源不可讀、目的地衝突),manifest 已經遷移完成、版本已前進,而其他一件都沒裝。**升級的第一步就違反了它自己的排序規則。** |
| 2 | **`record_manifest` 不是原子的,而且沒有復原副本。** `grep -v > tmp` → `cat "$tmp" > "$MANIFEST_FILE"`(**截斷**)→ `rm -f tmp` → `echo >> file`(`install.sh:313-322`)。 | 讀實作即證 | 中斷在截斷與追加之間會**丟掉那個鍵**;暫存檔在追加前就刪了。每次安裝呼叫約九次,所以視窗被踩九次。而 manifest 是解除安裝唯一的「存在什麼」紀錄 —— 丟一個鍵就是漏掉一個產物。 |
| 3 | **`--status` 的 `wired` 比它聽起來弱。** `installed_command`(`settings.rs:504-523`)回傳第一個 `is_ours_for` 通過的**指令字串**,從不比對 `matcher`、`timeout` 或分組。 | 讀實作即證 | 它確立「存在一個認得出是我們的條目」,**不是**「這個條目等於這個版本會安裝的形狀」。指令正確而 matcher 過時的 hook 會回報 `wired`。升級必須回答後者,所以計畫三的診斷不能建立在這個答案上。 |
| 4 | **skill 與它的 stamp 不是單一可復原單元。** `write_skill` 是 `cat "$1" > "$2"`(截斷再寫)**然後**才分開寫 stamp(`install.sh:216-219`)。 | 讀實作即證 | 寫到一半崩潰留下半個 SKILL.md,而舊 stamp 的雜湊不再吻合 → 下次 preflight 判定為**未管理的衝突**並拒絕碰它。**崩潰把一個受管理的檔案變成安裝器拒絕修復的檔案**,使用者得用 `--force`。 |
| 5 | **安裝器層級完全沒有並行控制。** `install.sh` 裡沒有任何 `flock`、lockfile 或 mkdir-lock。 | `grep -nE "flock\|lockfile\|\.lock"` 零命中 | 兩個並行的 `./install.sh` 可以在 payload 刪除/複製、manifest 改寫、以及 hook 檔的 read-modify-write 之間自由交錯。這正是 §3(b) 那兩把 flock 要解決的問題的另一半 —— 它們防的是升級與 hook 之間,這一條是升級與升級之間。 |

**共同的形狀值得記:五條裡有四條是「先毀掉舊狀態,再建立新狀態」。** manifest 的截斷再追加、skill 的截斷再蓋章、payload 的 `rm -rf` 再複製 —— 都是同一個反模式,而計畫三的 stage → validate → activate → cleanup 正是它的解藥。把它們當成五個獨立缺陷去修會做五次;把它們當成一個排序原則去修,只做一次。
...
| 2 | ~~**`extractor_version` 在資料庫裡存兩份。**~~ **讀取端已於 2026-09-06 收斂**(`status_of` 改讀 `_cortex_meta`);欄位已無讀者,移除待計畫三。原文: `projects.extractor_version` 欄位與 `_cortex_meta` 的同名鍵,由 `indexer.rs:436-437` 與 `:377-381` 在同一交易寫入。今天兩者必然一致,因為只有一個寫者。 | **計畫三** | 這是資料庫內部的反漂移違規,與 §1 那三條同類(shim 樣板、ast-grep 釘選、manifest key-set),應該一起收斂到單一的家。分開修會做兩次遷移。 |
| 3 | ~~**`pack.rs` 的 `walk()` 對失敗全部安靜跳過**~~ **已於 2026-09-06 修正**:`pack_files` 與 `extractor_version` 回傳失敗。原文:,於是半個 pack 會算出看起來合法的假 extractor identity(§3a 已載)。 | **計畫三** | 由第 3 步「驗證新世代完整、拒絕不完整的 pack」涵蓋。但 `walk()` 本身的安靜跳過應一併改成回傳失敗 —— §3a 只擋住了升級這條路徑,任何其他呼叫者仍會拿到短掉的清單。 |
| 4 | ~~Codex 首輪五條未核實~~ → **已於 2026-09-06 全部核實成立**,見 §9。 | **計畫三** | 五條裡有四條是同一個反模式:先毀掉舊狀態再建立新狀態。當成一個排序原則修,不是五個缺陷。 |
