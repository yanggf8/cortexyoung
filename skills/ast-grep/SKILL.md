...
4. **Trace callers or blast radius** — only for an explicit relationship question **in TypeScript, TSX, JavaScript or Python**, use `cort impact --symbol <name[,name2]> --depth <n> -f lean`. Use `--depth 1` for direct callers and the default depth 3 for a genuinely transitive question. Do not route ordinary lookup here merely because a graph is available. For **Rust** callers, do not use `impact`: the Rust rule pack indexes symbols but not call edges, so `dependents=0` means "no edges indexed", not "nothing depends on this". Use `rg` for one hop and the compiler (`cargo check`) for the precise list.
...
Before trusting indexed answers, check `stale=` in lean output or `index_is_stale` in JSON. If true, run `cort index --incremental` first or fall back to `rg`. Staleness covers two things: a file whose indexed content no longer matches disk, and a relationship graph that has not been recomputed since its chunks changed (a `graph_pending` marker set by an interrupted or upgraded index — the next index rebuilds it and clears the flag). A new untracked file is invisible until indexed. `cort read` and `cort recall` do not use that field; they validate stored readings against the source instead.
