...
export class CachedEmbedder extends ProcessPoolEmbedder {
  ...
  constructor(repositoryPath: string, processCount?: number) {
    ...
    const globalCacheDir = path.join(require('os').homedir(), '.claude', 'cortex-embeddings', repoHash);
  ...
  async generateEmbeddings(chunks: CodeChunk[]): Promise<CodeChunk[]> {
    ...
    if (partition.uncached.length > 0) {
      ...
      await this.updateCache(newEmbeddings);
    }
  ...
  private async updateCache(newChunks: CodeChunk[]): Promise<void> {
